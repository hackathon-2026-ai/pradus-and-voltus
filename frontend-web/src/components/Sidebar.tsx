...
const Sidebar: FC<SidebarProps> = ({
  ...
  const navItems = [
    { section: 'map', label: t('nav.mapView'), icon: (
    ...
    { section: 'analytics', label: t('nav.analytics'), icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
    )},
    { section: 'compare', label: t('nav.compare'), icon: (
    ...
    >
      ...
      <div className="sidebar-footer">
        <div className="zoom-info">
          <span className="zoom-label">{t('nav.zoom')}</span>
          <span className="zoom-value" id="zoom-level">{zoomLevel}</span>
        </div>
        <a
          href="#"
          className={`nav-item nav-item-chat${chatOpen ? ' active' : ''}`}
          id="nav-chat"
          onClick={(e) => { e.preventDefault(); onChatToggle(); }}
          onMouseMove={handleMouseMove}
          onMouseLeave={handleMouseLeave}
        >
          ...
          <span className="nav-label">{t('nav.aiChat')}</span>
          <div className="nav-indicator"></div>
        </a>
        <a
          href="#"
        ...
        >
          ...
          <span className="nav-label">{t('nav.flyToPoland')}</span>
          <div className="nav-indicator"></div>
        </a>
        ...
        <button className="sidebar-collapse-btn" onClick={onCollapseToggle} aria-label={collapsed ? t('nav.expandSidebar') : t('nav.collapseSidebar')}>
